...
struct PushConstants { int dummy; };
...
private:
    ...
    static void cursorPosCallback(GLFWwindow* w, double xpos, double ypos) {
        ...
        // Pan (shift + right, or middle mouse)
    ...
    // ---------- NEW: chunked buffer‑to‑image copy ----------
    void copyBufferToImageRegion(VkBuffer buffer, VkImage image,
        ...
        region.bufferOffset = 0;                         // staging buffer always starts at offset 0
    ...
    void initVulkan() {
        ...
        createCommandPool();
        prepareSphere();
        loadTexture();               // <-- corrected with size_t
    ...
    // ---------- FIXED: chunked texture loading with size_t ----------
    void loadTexture() {
        ...
        // Use size_t to avoid integer overflow for huge textures
        size_t w = ds->GetRasterXSize();
        size_t h = ds->GetRasterYSize();
        std::cout << "📂 Texture: " << w << " x " << h << "\n";

        const int nBands = 3;   // small – no overflow risk
        std::vector<uint8_t> rgb(w * h * nBands);   // size_t arithmetic

        int bandMap[3] = {1, 2, 3};
        // GDAL RasterIO expects int width/height – cast is safe (w, h <= 32768)
        ...
        // Create the GPU image (optimal tiling, device-local)
        ...
        // Chunked upload using a small staging buffer (64 MiB)
        ...
        uint32_t maxRowsPerChunk = (uint32_t)(STAGING_SIZE / (w * 4));   // w is size_t, safe
    ...
    // --- Uniform buffers, descriptors, command buffers, sync, main loop unchanged ---
